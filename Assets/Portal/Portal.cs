...
public class Portal : MonoBehaviour {
   ...
   static readonly int DisplayMask = Shader.PropertyToID("displayMask");
   ...
   void HandlePlayerTeleport() {
         ...
         PortalTraveller traveller = playerTraveller;
         Transform travellerT = traveller.transform;
         var m = linkedPortal.transform.localToWorldMatrix * transform.worldToLocalMatrix * travellerT.localToWorldMatrix;

         //Debug.DrawLine(transform.position, transform.position + transform.forward, Color.red, 10f);

         Vector3 offsetFromPortal = travellerT.position - transform.position;
         if(offsetFromPortal.magnitude<1){
         int portalSide = System.Math.Sign (Vector3.Dot (offsetFromPortal, transform.forward));
         int portalSideOld = System.Math.Sign (Vector3.Dot (traveller.previousOffsetFromPortal, transform.forward));
         // Teleport the traveller if it has crossed from one side of the portal to the other
         if (portalSide != portalSideOld) {
